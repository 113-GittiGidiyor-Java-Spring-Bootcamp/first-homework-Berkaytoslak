...
public class CourseService implements CrudRepository<Course>, CourseRepository {
    ...
    public void deleteFromDatabase(Course course) {
        ...
        try {
            entityManager.getTransaction().begin();
            Course foundCourse = entityManager.createQuery("from Course c where c.courseCod =: courseCod", Course.class).setParameter("courseCod", course.getCourseCod()).getSingleResult();
            entityManager.remove(foundCourse);
            entityManager.getTransaction().commit();
        }catch (Exception e){
            entityManager.getTransaction().rollback();
    ...
    public void updateOnDatabase(Course course, int id) {
        ...
        try {
            ...
            updateCourse.setCourseCod(course.getCourseCod());
